...
const elements = {
  ...
  runtimeSummary: $('runtime-summary'), previewStage: $('preview-exact-root'), previewStageShell: $('preview-stage-shell'),
...
function speedSliderToRate(value) { return Math.round(Math.pow(2, (50 - Number(value)) / 50) * 100) / 100; }
function rateToSpeedSlider(rate) { return Math.round(50 - Math.log2(Math.max(0.01, Number(rate) || 1)) * 50); }
function formatSpeed(rate) { const r = Math.round(Number(rate || 1) * 100) / 100; if (r === 1) return '1×'; return r.toFixed(2).replace(/\.?0+$/, '') + '×'; }
...
function getSlideEffectiveDurationMs(slide) {
  ...
  if (isVideoSlide(slide)) {
    ...
    const playbackRate = Number(slide.playbackRate || 1);
...
function schedulePreviewFrameUpdate(delay = 90) { clearTimeout(previewUpdateTimerId); previewUpdateTimerId = setTimeout(() => { if (previewUpdateRafId) return; previewUpdateRafId = window.requestAnimationFrame(() => { previewUpdateRafId = null; renderPreviewFrame(); }); }, delay); }
function updateSlideTimingUi(card, slide) { const durationEl = card.querySelector('[data-used-duration]'); if (durationEl) durationEl.textContent = getSlideEffectiveDurationMs(slide) > 0 ? formatDuration(getSlideEffectiveDurationMs(slide)) : 'غير معروفة'; updateRuntimeSummary(); schedulePreviewFrameUpdate(); }
...
function createSlideCard(slide, index) {
  ...
  const mediaUrl = slide.fileUrl || toFileUrl(slide.imagePath);
  const isVideo = slide.mediaType === 'video' || isVideoPath(slide.imagePath);
  const trimStartSec = Number(slide.trimStartMs || 0) / 1000;
  const trimEndSec = slide.trimEndMs ? Number(slide.trimEndMs) / 1000 : '';
  const maxSec = Number(slide.mediaDurationMs || 0) / 1000;
  const imageDurationSec = (slide.durationMs > 0 ? Number(slide.durationMs) : DEFAULT_IMAGE_DURATION_MS) / 1000;
  const playbackRate = Number(slide.playbackRate || 1);
  const speedSliderValue = rateToSpeedSlider(playbackRate);
  const speedLabel = formatSpeed(playbackRate);
  const usedDurationMs = getSlideEffectiveDurationMs(slide);

  card.className = `slide-card ${isVideo ? 'video-slide-card' : ''} ${slide.id === state.selectedSlideId ? 'selected' : ''} ${slide.id === state.dragOverSlideId ? 'drag-over' : ''}`;
  card.dataset.slideId = slide.id;
  card.draggable = false;
  card.innerHTML = isVideo ? `
    <div class="slide-side-meta"><div class="slide-order">${String(index + 1).padStart(2, '0')}</div><div class="drag-handle" title="اسحب لإعادة الترتيب"><span></span><span></span><span></span></div></div>
    <div class="slide-thumb-wrap video-thumb-wrap" title="${escapeHtml(prettifyPath(slide.imagePath))}"><video class="slide-thumb" src="${mediaUrl}" muted preload="metadata"></video><button class="thumb-mute-btn" data-action="mute-toggle" title="${slide.isMuted !== false ? 'تشغيل الصوت' : 'كتم الصوت'}"><i data-lucide="${slide.isMuted !== false ? 'volume-x' : 'volume-2'}"></i></button></div>
    <label class="trim-compact-field"><span>in</span><input class="number-input" type="number" min="0" ${maxSec > 0 ? `max="${maxSec.toFixed(1)}"` : ''} step="0.1" data-trim="start" value="${trimStartSec.toFixed(1)}" /></label>
    <label class="trim-compact-field"><span>out</span><input class="number-input" type="number" min="0" ${maxSec > 0 ? `max="${maxSec.toFixed(1)}"` : ''} step="0.1" data-trim="end" placeholder="${maxSec > 0 ? maxSec.toFixed(1) : 'full'}" value="${trimEndSec === '' ? '' : trimEndSec.toFixed(1)}" /></label>
    <div class="video-speed-row"><input type="range" min="0" max="100" value="${speedSliderValue}" dir="ltr" style="flex:1;accent-color:var(--accent);cursor:pointer" data-speed-slider /><span data-speed-label style="font-size:.72rem;font-family:var(--font-mono);font-weight:800;min-width:28px;text-align:center;direction:ltr">${speedLabel}</span></div>
    <button class="btn-secondary slide-icon-action" data-action="replace" title="استبدال" aria-label="استبدال"><i data-lucide="replace"></i></button>
    <button class="btn-secondary btn-danger slide-icon-action" data-action="delete" title="حذف" aria-label="حذف"><i data-lucide="trash-2"></i></button>`
    : `
    <div class="slide-side-meta"><div class="slide-order">${String(index + 1).padStart(2, '0')}</div><div class="drag-handle" title="اسحب لإعادة الترتيب"><span></span><span></span><span></span></div></div>
    <div class="slide-thumb-wrap"><img class="slide-thumb" src="${mediaUrl}" alt="slide" /></div>
    <div class="slide-main"><div class="slide-title">${prettifyPath(slide.imagePath) || `شريحة ${index + 1}`}</div><div class="slide-meta"><span class="badge">صورة</span></div><div style="display:flex;align-items:center;gap:.5rem"><label class="muted-text" style="white-space:nowrap">مدة الصورة (ث):</label><input class="number-input slide-duration-input" type="number" min="1" step="1" value="${imageDurationSec}" style="width:70px;text-align:center" /></div></div>
    <div class="slide-actions-stack"><button class="btn-secondary" data-action="replace" title="استبدال"><i data-lucide="replace"></i></button><button class="btn-secondary btn-danger" data-action="delete" title="حذف"><i data-lucide="trash-2"></i></button></div>`;

  card.addEventListener('click', (event) => {
    if (event.target.closest('button,input,select,textarea,label')) return;
    if (state.selectedSlideId === slide.id) return;
    state.selectedSlideId = slide.id;
    renderSlides();
  });
  card.querySelectorAll('button,input,select,textarea,label').forEach((control) => {
  ...
  card.querySelectorAll('[data-trim]').forEach((input) => {
    input.addEventListener('input', () => {
      const target = state.slides.find((s) => s.id === slide.id);
      if (!target) return;
      if (input.dataset.trim === 'start') target.trimStartMs = Math.max(0, Number(input.value || 0) * 1000);
      if (input.dataset.trim === 'end') target.trimEndMs = input.value === '' ? null : Math.max(0, Number(input.value || 0) * 1000);
      if (target.trimEndMs && target.trimEndMs <= (target.trimStartMs || 0)) {
        input.setCustomValidity('النهاية يجب أن تكون أكبر من البداية');
      } else {
        input.setCustomValidity('');
      }
      markProjectDirty();
      updateSlideTimingUi(card, target);
    });
    input.addEventListener('change', () => updateSlideTimingUi(card, state.slides.find((s) => s.id === slide.id) || slide));
  });
  card.querySelectorAll('.slide-duration-input').forEach((input) => {
    input.addEventListener('input', () => {
      const target = state.slides.find((s) => s.id === slide.id);
      if (!target) return;
      const seconds = Math.max(1, Number(input.value || 8));
      target.durationMs = seconds * 1000;
      markProjectDirty();
      updateRuntimeSummary();
      schedulePreviewFrameUpdate();
    });
  });
  card.querySelector('[data-action="mute-toggle"]')?.addEventListener('click', (event) => { event.stopPropagation(); const target = state.slides.find((s) => s.id === slide.id); if (!target) return; target.isMuted = (target.isMuted === false); const btn = event.currentTarget; const isMuted = target.isMuted !== false; btn.title = isMuted ? 'تشغيل الصوت' : 'كتم الصوت'; btn.innerHTML = `<i data-lucide="${isMuted ? 'volume-x' : 'volume-2'}"></i>`; if (window.lucide) window.lucide.createIcons({ nodes: [btn] }); markProjectDirty(); schedulePreviewFrameUpdate(); });
  card.querySelector('[data-speed-slider]')?.addEventListener('input', (event) => { const target = state.slides.find((s) => s.id === slide.id); if (!target) return; const rate = speedSliderToRate(event.currentTarget.value); target.playbackRate = rate; const labelEl = card.querySelector('[data-speed-label]'); if (labelEl) labelEl.textContent = formatSpeed(rate); updateSlideTimingUi(card, target); markProjectDirty(); });
  const handle = card.querySelector('.drag-handle');
  handle?.addEventListener('dragstart', (event) => { state.draggedSlideId = slide.id; state.dragOverSlideId = slide.id; if (event.dataTransfer) { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', slide.id); } });
  handle?.addEventListener('dragend', () => { state.draggedSlideId = null; state.dragOverSlideId = null; renderSlides(); });
  if (handle) handle.draggable = true;
  card.addEventListener('dragover', (event) => { if (!state.draggedSlideId || state.draggedSlideId === slide.id) return; event.preventDefault(); state.dragOverSlideId = slide.id; card.classList.add('drag-over'); });
  card.addEventListener('drop', (event) => { event.preventDefault(); if (!state.draggedSlideId || state.draggedSlideId === slide.id) return; reorderSlides(state.draggedSlideId, slide.id); state.draggedSlideId = null; state.dragOverSlideId = null; markProjectDirty(); renderSlides(); });
  setTimeout(() => { if (window.lucide) window.lucide.createIcons({ nodes: [card] }); }, 0);
  return card;
}
async function applyPickedMediaToSlide(slideId, media) { const target = state.slides.find((s) => s.id === slideId); if (!target) return; const mediaType = isVideoPath(media.imagePath) ? 'video' : 'image'; const fileUrl = media.fileUrl || toFileUrl(media.imagePath); if (mediaType === 'image') { const oldDuration = target.mediaType === 'image' ? (target.durationMs || DEFAULT_IMAGE_DURATION_MS) : DEFAULT_IMAGE_DURATION_MS; Object.assign(target, { imagePath: media.imagePath, fileUrl, mediaType, isMuted: true, trimStartMs: 0, trimEndMs: null, playbackRate: 1, durationMs: oldDuration, mediaDurationMs: 0 }); } else { const mediaDurationMs = await readMediaDurationMs(fileUrl, 0); Object.assign(target, { imagePath: media.imagePath, fileUrl, mediaType, isMuted: true, trimStartMs: 0, trimEndMs: null, playbackRate: 1, durationMs: mediaDurationMs, mediaDurationMs }); } markProjectDirty(); renderSlides(); }
...
function renderPreviewFrame() { const api = previewApi(); if (!api || !elements.previewStage) return; const inputProps = buildPreviewInputProps(); const frames = getTotalDurationFrames(); if (!previewMounted) { api.mount(elements.previewStage); previewMounted = true; } api.update({ inputProps, durationInFrames: frames }); }
