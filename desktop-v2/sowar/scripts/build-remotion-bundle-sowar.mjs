...
async function run() {
  ...
  const entryPoint = path.join(repoRoot, 'src', 'remotion', 'sowar', 'index.ts');
  const stagedPublicDir = await prepareRemotionPublicDir(desktopPaths);
  const outDir = process.env.DESKTOP_V2_BUNDLE_OUT_DIR_SOWAR || path.join(buildRoot, 'bundle-staging-sowar', 'remotion-bundle');
  ...
  console.log('Desktop V2 Sowar Remotion bundle ready at:', serveUrl);
}

run().catch((error) => {
  console.error('Desktop V2 Sowar bundle build failed:', error);
  process.exit(1);
});
