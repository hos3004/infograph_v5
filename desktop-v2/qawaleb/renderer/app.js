...
const state = {
  ...
  appVersion: '1.0.0',
  defaultBackgroundPreviewDataUrl: '',
  project: {
    currentProjectPath: '',
  ...
  backgroundRadius: DEFAULT_BACKGROUND_RADIUS,
  backgroundFeather: DEFAULT_BACKGROUND_FEATHER,
...
const elements = {
  ...
  portraitScaleInput: document.getElementById('portrait-scale-input'),
  portraitScaleValue: document.getElementById('portrait-scale-value'),
  portraitXInput: document.getElementById('portrait-x-input'),
  portraitXValue: document.getElementById('portrait-x-value'),
  portraitYInput: document.getElementById('portrait-y-input'),
  portraitYValue: document.getElementById('portrait-y-value'),
  quoteMarkVisibleCheckbox: document.getElementById('quote-mark-visible-checkbox'),
  ...
  templateImageControlsSection: document.getElementById('template-image-controls-section'),
  templateImageControlsCard: document.getElementById('template-image-controls-card'),
  templateBackgroundControlsSection: document.getElementById('template-background-controls-section'),
  templateBackgroundControlsCard: document.getElementById('template-background-controls-card'),
...
function updateTemplateRuntimeSummary() {
  ...
  if (elements.runtimeSummary) {
    elements.runtimeSummary.textContent = preset ? `${durationSeconds}ث` : '';
    elements.runtimeSummary.title = preset ? `المدة التجريبية: ${durationSeconds} ثانية` : '';
  }
...
function getTemplatePreviewSignature(preset) {
  return JSON.stringify({
    ...
    portraitX: state.portraitX,
    portraitY: state.portraitY,
    portraitMonochrome: state.portraitMonochrome !== false,
    portraitSquare: state.portraitSquare === true,
    showQuoteMark: state.showQuoteMark !== false,
...
function templateSupportsPortraitControls(templateId = state.selectedTemplateId) {
  return templateId === 'points-broadcast';
}
...
function buildTemplateTextFromValues(preset, values) {
  ...
  return preset.fields
    .filter((field) => field.includeInText !== false)
...
function buildTemplateImageFieldCard(field, value) {
  ...
  label.textContent = field.label?.replace('رابط ', '').trim() || 'صورة';
  ...
  hint.textContent = 'القوالب تستخدم صورًا محلية من داخل التطبيق، وليس روابط خارجية.';
...
function renderTemplateImageControls() {
  ...
  const hint = document.createElement('span');
  hint.className = 'template-field-hint';
  hint.textContent = supportsPortraitControls
    ? 'كل خصائص الصورة مجمعة هنا: التحميل، الحجم، التحريك، والشكل.'
    : 'هذا القالب يدعم حاليًا تحميل صورة محلية من داخل التطبيق.';
  container.appendChild(hint);

  if (!supportsPortraitControls) {
    return;
  }
  ...
  controlsWrap.style.gap = '0.85rem';
  controlsWrap.style.marginTop = '0.95rem';

  const appendSliderControl = ({ label, valueText, min, max, step, value, onInput }) => {
    const wrapper = document.createElement('div');
    wrapper.className = 'text-slider-control';

    const head = document.createElement('div');
    head.className = 'text-slider-head';
    ...
    input.addEventListener('input', (event) => {
      ...
      const nextLabel = onInput(nextValue);
  ...
  const appendSwitchControl = ({ label, checked, onChange }) => {
    const row = document.createElement('label');
    row.className = 'parallax-toggle text-switch-row';
  ...
  appendSliderControl({
    label: 'تكبير وتصغير محتوى الصورة',
    valueText: `${Math.round(state.portraitScale * 100)}%`,
    min: 70,
    max: 170,
    step: 1,
    value: Math.round(state.portraitScale * 100),
    onInput: (nextValue) => {
      state.portraitScale = nextValue / 100;
      markProjectDirty();
      renderPreviewFrame();
      return `${Math.round(state.portraitScale * 100)}%`;
    },
  });

  appendSliderControl({
    label: 'تحريك محتوى الصورة يمينًا ويسارًا',
    valueText: `${state.portraitX > 0 ? '+' : ''}${state.portraitX}px`,
    min: -220,
    max: 220,
    step: 2,
    value: state.portraitX,
    onInput: (nextValue) => {
      state.portraitX = nextValue;
      markProjectDirty();
      renderPreviewFrame();
      return `${state.portraitX > 0 ? '+' : ''}${state.portraitX}px`;
    },
  });

  appendSliderControl({
    label: 'تحريك محتوى الصورة أعلى وأسفل',
    valueText: `${state.portraitY > 0 ? '+' : ''}${state.portraitY}px`,
    min: -220,
    max: 220,
    step: 2,
    value: state.portraitY,
    onInput: (nextValue) => {
      state.portraitY = nextValue;
      markProjectDirty();
      renderPreviewFrame();
      return `${state.portraitY > 0 ? '+' : ''}${state.portraitY}px`;
    },
  });

  appendSwitchControl({
    label: 'أبيض وأسود',
    checked: state.portraitMonochrome !== false,
    onChange: (checked) => {
      state.portraitMonochrome = checked;
      markProjectDirty();
      renderPreviewFrame();
    },
  });
  ...
  appendSwitchControl({
    label: 'شكل مربع',
    checked: state.portraitSquare === true,
    onChange: (checked) => {
      state.portraitSquare = checked;
      markProjectDirty();
      renderPreviewFrame();
    },
  });
...
function renderTemplateBackgroundControls() {
  const section = elements.templateBackgroundControlsSection;
  const container = elements.templateBackgroundControlsCard;
  ...
  const customPath = sanitizeTemplateImageValue(state.backgroundImage || '');
  const previewSrc = customPath
    ? window.desktopApi.toFileUrl(customPath)
    : (state.defaultBackgroundPreviewDataUrl || '');

  const title = document.createElement('label');
  title.className = 'template-field-label';
  title.textContent = 'الخلفية المتحركة';
  container.appendChild(title);
  ...
  if (previewSrc) {
    const img = document.createElement('img');
    img.src = previewSrc;
    img.alt = customPath ? 'خلفية مخصصة' : 'الخلفية الافتراضية';
    img.style.width = '100%';
    img.style.maxHeight = '260px';
    img.style.objectFit = 'cover';
    img.style.display = 'block';
    ...
    badge.className = 'template-field-hint';
    ...
    badge.style.background = 'rgba(0,0,0,0.58)';
    badge.style.color = '#fff';
    badge.style.backdropFilter = 'blur(6px)';
    badge.textContent = customPath ? 'خلفية مخصصة' : 'الخلفية الافتراضية';
    ...
    empty.style.padding = '1rem';
    empty.style.textAlign = 'center';
    empty.textContent = 'الخلفية الافتراضية مفعلة، ويمكنك استبدالها بصورة محلية.';
    previewBox.appendChild(empty);
  }
  container.appendChild(previewBox);
  ...
  fileLabel.style.textOverflow = 'ellipsis';
  fileLabel.style.whiteSpace = 'nowrap';
  fileLabel.textContent = customPath ? prettifyPath(customPath) : 'الخلفية الافتراضية';
  fileLabel.title = customPath || 'الخلفية الافتراضية';
  ...
  pickBtn.type = 'button';
  pickBtn.className = 'btn-secondary';
  pickBtn.textContent = customPath ? 'استبدال الخلفية' : 'اختيار خلفية';
  pickBtn.addEventListener('click', async () => {
    const pickedImage = await (window.desktopApi.pickTemplateImage?.() || window.desktopApi.pickSlideImage());
    if (!pickedImage) {
      return;
    ...
    const nextPath = sanitizeTemplateImageValue(pickedImage.imagePath || pickedImage.path || '');
    if (!nextPath || !isSupportedSlideImage(nextPath)) {
      setStatus('تنبيه', 'امتداد الخلفية غير مدعوم. استخدم PNG أو JPG أو JPEG أو WEBP.');
      return;
    }

    state.backgroundImage = nextPath;
    ensureTemplateSlide();
    markProjectDirty();
    renderTemplateBackgroundControls();
    renderPreviewFrame();
  });

  const resetBtn = document.createElement('button');
  resetBtn.type = 'button';
  resetBtn.className = 'btn-blue-outline';
  resetBtn.textContent = 'العودة للافتراضي';
  resetBtn.disabled = !customPath;
  ...
  const hint = document.createElement('span');
  hint.className = 'template-field-hint';
  hint.textContent = 'تتحرك هذه الطبقة بخفة مع خيار Parallax العام، وتُستخدم كلمسة بصرية فوق خلفية القالب.';
  container.appendChild(hint);
  ...
  appendSliderControl({
    ...
    valueText: `${Math.round(state.backgroundOpacity)}%`,
    min: 0,
    max: 40,
    step: 1,
    value: Math.round(state.backgroundOpacity),
    onInput: (nextValue) => {
      state.backgroundOpacity = clamp(nextValue, 0, 100);
      markProjectDirty();
      renderPreviewFrame();
      return `${Math.round(state.backgroundOpacity)}%`;
    },
  });

  appendSliderControl({
    label: 'ضبابية الخلفية',
    valueText: `${Math.round(state.backgroundBlur)}px`,
    min: 0,
    max: 40,
    step: 1,
    value: Math.round(state.backgroundBlur),
    onInput: (nextValue) => {
      state.backgroundBlur = clamp(nextValue, 0, 40);
      markProjectDirty();
      renderPreviewFrame();
      return `${Math.round(state.backgroundBlur)}px`;
    },
  });

  appendSliderControl({
    label: 'دائرية الحواف',
    valueText: `${Math.round(state.backgroundRadius)}px`,
    min: 0,
    max: 240,
    step: 2,
    value: Math.round(state.backgroundRadius),
    onInput: (nextValue) => {
      state.backgroundRadius = clamp(nextValue, 0, 240);
      markProjectDirty();
      renderPreviewFrame();
      return `${Math.round(state.backgroundRadius)}px`;
    },
  });

  appendSliderControl({
    label: 'نعومة الحدود',
    valueText: `${Math.round(state.backgroundFeather)}px`,
    min: 0,
    max: 180,
    step: 2,
    value: Math.round(state.backgroundFeather),
    onInput: (nextValue) => {
      state.backgroundFeather = clamp(nextValue, 0, 180);
      markProjectDirty();
      renderPreviewFrame();
      return `${Math.round(state.backgroundFeather)}px`;
    },
  });
...
async function applyOpenedProject(project, filePath) {
  ...
  try {
    ...
    state.backgroundImage = sanitizeTemplateImageValue(appearance.backgroundImage || '');
    state.backgroundOpacity = clamp(Number(appearance.backgroundOpacity ?? DEFAULT_BACKGROUND_OPACITY), 0, 100);
    state.backgroundBlur = clamp(Number(appearance.backgroundBlur ?? DEFAULT_BACKGROUND_BLUR), 0, 40);
    state.backgroundRadius = clamp(Number(appearance.backgroundRadius ?? DEFAULT_BACKGROUND_RADIUS), 0, 240);
    state.backgroundFeather = clamp(Number(appearance.backgroundFeather ?? DEFAULT_BACKGROUND_FEATHER), 0, 180);
    ...
    state.portraitScale = Math.max(0.7, Math.min(1.7, Number(appearance.portraitScale || DEFAULT_PORTRAIT_SCALE)));
    state.portraitX = Number(appearance.portraitX || 0);
    state.portraitY = Number(appearance.portraitY || 0);
    state.portraitMonochrome = appearance.portraitMonochrome !== false;
    state.portraitSquare = appearance.portraitSquare === true;
    state.showQuoteMark = appearance.showQuoteMark !== false;
...
function updateVoiceoverMeta() {
  ...
  elements.voiceoverMeta.textContent = `${durationSec.toFixed(1)}ث (~${slidesNeeded} مشهد)`;
}
...
function renderTemplateColorControls() {
  ...
  if (!colorDefinitions.length) {
    ...
    emptyState.textContent = 'لا توجد ألوان قابلة للتخصيص لهذا القالب حاليًا.';
...
function syncTextSettingsUi() {
  ...
  if (elements.portraitScaleInput && elements.portraitScaleValue) {
    const portraitScalePercent = Math.round(state.portraitScale * 100);
    elements.portraitScaleInput.value = String(portraitScalePercent);
    elements.portraitScaleValue.textContent = `${portraitScalePercent}%`;
    updateRangeVisual(elements.portraitScaleInput);
  }

  if (elements.portraitXInput && elements.portraitXValue) {
    elements.portraitXInput.value = String(state.portraitX);
    elements.portraitXValue.textContent = `${state.portraitX > 0 ? '+' : ''}${state.portraitX}px`;
    updateRangeVisual(elements.portraitXInput);
  }

  if (elements.portraitYInput && elements.portraitYValue) {
    elements.portraitYInput.value = String(state.portraitY);
    elements.portraitYValue.textContent = `${state.portraitY > 0 ? '+' : ''}${state.portraitY}px`;
    updateRangeVisual(elements.portraitYInput);
  }

  if (elements.quoteMarkVisibleCheckbox) {
    elements.quoteMarkVisibleCheckbox.checked = state.showQuoteMark !== false;
  }
...
function buildTemplatePreviewInputProps() {
  ...
  const previewBackgroundUrl = normalizeTemplatePreviewValue(
    state.backgroundImage || state.defaultBackgroundPreviewDataUrl || '',
  );
  ...
  return {
    ...
    backgroundFeather: Number(state.backgroundFeather ?? DEFAULT_BACKGROUND_FEATHER),
    parallaxEnabled: state.parallaxEnabled !== false,
    templateColors: { ...state.templateColors },
    ...
    portraitScale: Number(state.portraitScale || DEFAULT_PORTRAIT_SCALE),
    portraitX: Number(state.portraitX || 0),
    portraitY: Number(state.portraitY || 0),
    portraitMonochrome: state.portraitMonochrome !== false,
    portraitSquare: state.portraitSquare === true,
    showQuoteMark: state.showQuoteMark !== false,
...
function renderExactPreview() {
  ...
  if (hasExactPreviewPlayer()) {
    ...
    if (!exactPreviewRef.playerMounted) {
      api.mount(mountNode);
      exactPreviewRef.playerMounted = true;
    }
    api.update({
...
function renderSlides() {
  ...
  const visibleFields = preset.fields.filter((field) => !isTemplateImageField(field));
  if (elements.slidesCount) {
    elements.slidesCount.textContent = String(visibleFields.length);
  }
...
async function bootstrap() {
  ...
  state.defaultBackgroundPreviewDataUrl = bootstrapPayload.qawalebDefaultBackgroundDataUrl || '';
...
if (elements.portraitScaleInput) {
  elements.portraitScaleInput.addEventListener('input', (event) => {
    state.portraitScale = Number(event.target.value) / 100;
    syncTextSettingsUi();
    renderPreviewFrame();
  });
}

if (elements.portraitXInput) {
  elements.portraitXInput.addEventListener('input', (event) => {
    state.portraitX = Number(event.target.value);
    syncTextSettingsUi();
    renderPreviewFrame();
  });
}

if (elements.portraitYInput) {
  elements.portraitYInput.addEventListener('input', (event) => {
    state.portraitY = Number(event.target.value);
    syncTextSettingsUi();
    renderPreviewFrame();
  });
}
...
if (elements.quoteMarkVisibleCheckbox) {
  elements.quoteMarkVisibleCheckbox.addEventListener('change', (event) => {
    state.showQuoteMark = event.target.checked;
    renderPreviewFrame();
  });
}
...
// Content generation controls live in content-generation.js.

// Prompt inspector, settings modal, and bootstrap startup live in settings-controls.js.
